...
private:
	float GetSFRValue(unsigned char *_InImageBuffer,ROIInfo roiinfo,int ROISize,wchar_t *stringinfo,int index,int blockIndex,int select_freque);
...
private:
	...
	BOOL m_bAFGoing;
	 ...
	 AFData m_stAFData[1024];   //记录全程中每个ROI的SFR值及其对应的DAC值
	 ...
	 float m_fCTC5_H_Fact;
	 float m_fCTC5_V_Fact;
	 bool bResult;
public:
...
private:
	...
	void AppendAfBurnFile(CString strAlgo, int cnt, int code, float roiVal[]);
	void SFR_Fov_6();
	float m_dSFRValue_6[MAXROICOUNT];	//记录当前DAC对应的各个ROI的SFR
	int m_iMiniStepIndex_6;
	BOOL bFlagCenterROI;
	BOOL Peak_Code_Check();
