...
{
	CString tempStr;
	int nPos = (pDlg->m_TestStation).Find('/');
	pDlg->m_TestStation = (nPos < 0) ? pDlg->m_TestStation : pDlg->m_TestStation.Left(nPos-1);
	tempStr.Format(_T("m_TestStation = %s m_AFStation = %d"),pDlg->m_TestStation, pDlg->m_AFStation);
	...
	//将字符串转换为整形
	_stscanf_s(pDlg->m_BufferSize, _T("%hX"), &usTemp);
	m_OTPSize = usTemp;
    ...
    if(pDlg->SaveEEPROMPDAF_En==1)
    {
        ...
        for(int i=0;i<8192;i++)
            tempotpdata[i]=uReadBuffer[i] & 0xFF;
	...
	nPos = (stAPI.m_StationName).Find('/');//修改根据点亮设定即界面选择的站名进行匹配相应的测试项 LYC 20180525
	stAPI.m_StationName = (nPos < 0) ? stAPI.m_StationName : stAPI.m_StationName.Left(nPos-1);
	...
	{
		m_pInterface->Ctrl_Addlog(CamID,_T("开始Check L476F00数据"),COLOR_BLUE,200);
		retValOtpCheck = stAPI.Testing(ErrorMsg, tempOTPBuff, m_OTPSize, 1,stAPI.m_StationName);
	} 
	else   //副摄
	{
		m_pInterface->Ctrl_Addlog(CamID,_T("开始Check副摄数据"),COLOR_BLUE,200);
	...
	if(retValOtpCheck != 1)
	{
		...
		temMsg=ErrorMsg;
		m_pInterface->Ctrl_Addlog(CamID,temMsg,COLOR_RED,200);
		m_pInterface->Ctrl_Addlog(CamID,sErrorInfo,COLOR_RED,200);
...
//	pDlg->m_AFStation   = 5;
	...
	if((pDlg->m_TestStation).Find(_T("SFR")) != -1)
	{
		pDlg->m_AFStation = 0;
	}
	...
	else if((pDlg->m_TestStation).Find(_T("标定")) != -1)
	{
		pDlg->m_AFStation = 4;
  //      pDlg->m_CalStation = TRUE;
	}
	else if((pDlg->m_TestStation).Find(_T("终检")) != -1)
	{
		pDlg->m_AFStation = 5;
...
int RX_OTPCheck_TestItem::SaveOption()
{
	...
	{
		pDlg->m_TestStation = " SFR   // 测试站位（目前仅支持：OTP、SFR、终检）";
	}
	else if (pDlg->m_AFStation == 1)
	{
		pDlg->m_TestStation = " OTP   // 测试站位（目前仅支持：OTP、SFR、终检）";
	}
	else if (pDlg->m_AFStation == 2)
	{
		pDlg->m_TestStation = " PDAF   // 测试站位（目前仅支持：OTP、SFR、PDAF、AEC、标定、终检）";
	}
	else if (pDlg->m_AFStation == 3)
	{
		pDlg->m_TestStation = "AEC   // 测试站位（目前仅支持：OTP、SFR、PDAF、AEC、标定、终检）";
	}
	else if (pDlg->m_AFStation == 4)
	{
		pDlg->m_TestStation = " 标定   // 测试站位（目前仅支持：OTP、SFR、PDAF、AEC、标定、终检）";
	}
	else if (pDlg->m_AFStation == 5)
	{
		pDlg->m_TestStation = " 终检   // 测试站位（目前仅支持：OTP、SFR、终检）";
	}
	
	else
	{
		pDlg->m_TestStation = " 终检   // 测试站位（目前仅支持：OTP、SFR、PDAF、AEC、标定、终检）";
	}
