	...
	int i=0;
	 {
		...
		temp.Format(_T("BLC=%d"),pDlg->m_BLC);
		m_pInterface->Ctrl_Addlog(CamID,temp,COLOR_BLUE,200);
		if(brightness>pDlg->m_BLC)
		{
			brightness -= pDlg->m_BLC;
		}
		else
		{
			brightness =0;
		...
		temp.Format(_T("减BLC 后brightness=%f <%d , %d>"),brightness,pDlg->m_LowLimit,pDlg->m_HighLimit);
		m_pInterface->Ctrl_Addlog(CamID,temp,COLOR_BLUE,200);
		...
		{
            //m_pInterface->Ctrl_SaveImage(CamID,FALSE,NULL,_T("AE"),3,NULL);
			m_pInterface->Sensor_ReadExp(CamID,exposure);
...
void AE_TestItem::CenterBlockRawInfo(USHORT *pIn,ImageInfo *mySensor ,int Ratio,int Ratio2,float &RawG,BOOL WhiteEnable)
{
	...
	if (pDlg->m_White128Enable)
	{
		if (pDlg->m_10bit)
		{
			m_pInterface->Ctrl_Addlog(CamID,_T("RawG = 512;"),COLOR_BLUE,200);
			RawG = 512;
		}
		else
		{
	    ...
	    CString OtpTxt_File = _T("rawg.ini");
	    CString g_temp= _T("");
	    CString myContent = _T("");
		...
		RawG  = int((Gr + Gb) / 2.0); 
		CString Temp;
		Temp.Format(_T("测试大于128的pixel... RawG = %f"),RawG);

		m_pInterface->Ctrl_Addlog(CamID,Temp,COLOR_BLUE,200);
		OutputDebugString(_T("测试大于128的pixel..."));
		CStdioFile myfile;
		if (myfile.Open(OtpTxt_File,CFile::modeCreate|CFile::modeWrite|CFile::typeText) == TRUE)
		{
			myfile.WriteString(myContent);
			myfile.Close();
		}
	...
	if (WhiteEnable)
	{
		...
		RawG  = int((Gr + Gb) / 2.0); 
		CString Temp;
		Temp.Format(_T("测试大于平均值的pixel... RawG = %f"),RawG);
		m_pInterface->Ctrl_Addlog(CamID,Temp,COLOR_BLUE,200);
