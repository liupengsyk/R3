...
int S5K3P3:: ReadReg(USHORT startAddr, USHORT endAddr, USHORT *buf,USHORT page)
{
	if(NULL == buf) return 0;
	if(I2cRead(startAddr,buf,3) != 1) return 0;
	return 1;
}
int S5K3P3::WriteReg(USHORT startAddr, USHORT endAddr, USHORT *buf,USHORT page)
{
	if(NULL == buf) return 0;
	if(I2cWrite(startAddr,*buf,3) != 1) return 0;
	return 1;
}
int S5K3P3::GetTemperature(USHORT &temperature)
{
	return 1;
}
int S5K3P3::GetFuseID(CString &FuseID) 
{
	FuseID = _T("");
	CString fuseid ;
	char section[512] = {0};
...
int S5K3P3::ApplyAWBGain(int rg, int bg, int Typical_rg, int Typical_bg)
{
...
int S5K3P3::ApplyLsc(BYTE* raw8,int width,int height,int nLSCTarget,int ob,UCHAR* nLenCReg,int nLenCRegCount,int LSCGroup)
{
	return 1;
}

int S5K3P3::spcCal(USHORT *imgBuf,short *pSPC)
{
	return 1;
}
int S5K3P3::ApplySpc(short *pSPC)
{
	return 1;
}

int S5K3P3::ReadExp( int &exp)
{
	USHORT ExpHigh;
	USHORT ExpLow;
...
int S5K3P3::ReadGain( USHORT &gain)
{
	USHORT GainHigh;
	USHORT GainLow;
...
int S5K3P3::WriteGain( USHORT gain)
{
