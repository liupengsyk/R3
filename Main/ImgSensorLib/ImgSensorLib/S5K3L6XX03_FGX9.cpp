...
S5K3L6XX03_FGX9::S5K3L6XX03_FGX9(void)
{
}
...
int S5K3L6XX03_FGX9:: ReadReg(USHORT startAddr, USHORT endAddr, USHORT *buf,USHORT page)
{
	return 1;
}
int S5K3L6XX03_FGX9::WriteReg(USHORT startAddr, USHORT endAddr, USHORT *buf,USHORT page)
{
	return 1;
}

int S5K3L6XX03_FGX9::GetTemperature(USHORT &temperature)
{
	return 1;
}
int S5K3L6XX03_FGX9::GetFuseID(CString &FuseID) 
{
	char section[512] = {0};
	USHORT tempVal[16];
	if(I2cWrite(0x0100,0x01,     3) != 1) return 0;   // 0x0101  «mirror flip …Ë÷√£¨        
	if(I2cWrite(0x0a02,0x0000,   4) != 1) return 0;   //page 0 CurrentSensor.mode           
	if(I2cWrite(0x0a00,0x0100,   4) != 1) return 0;   //read start                          
	Sleep(10);                                  
	if(I2cRead(0x0A04, tempVal,  4) != 1) return 0;                                         
	if(I2cRead(0x0A06, tempVal+1,4) != 1) return 0;                                         
	if(I2cRead(0x0A08, tempVal+2,4) != 1) return 0;                                         
	if(I2cRead(0x0A0A, tempVal+3,4) != 1) return 0;                                         
...
int S5K3L6XX03_FGX9::Init()
{
	return 1;
}
int S5K3L6XX03_FGX9::ApplyAWBGain(int rg, int bg, int Typical_rg, int Typical_bg)
{
...
#define GAIN_DEFAULT       0x0100
	...
	if(I2cWrite(0x3C90,0x00,  3) != 1) return 0;                   
...
int S5K3L6XX03_FGX9::ApplyLsc(BYTE* raw8,int width,int height,int nLSCTarget,int ob,UCHAR* nLenCReg,int nLenCRegCount,int LSCGroup)
{
	return 1;
}

int S5K3L6XX03_FGX9::spcCal(USHORT *imgBuf,short *pSPC)
{
	return 1;
}
int S5K3L6XX03_FGX9::ApplySpc(short *pSPC)
{
	return 1;
}

int S5K3L6XX03_FGX9::ReadExp( int &exp)
{
	USHORT ExpHigh;
	USHORT ExpLow;
...
int S5K3L6XX03_FGX9::WriteExp( int exp)
{
...
int S5K3L6XX03_FGX9::ReadGain( USHORT &gain)
{
	USHORT GainHigh;
	USHORT GainLow;
...
int S5K3L6XX03_FGX9::WriteGain( USHORT gain)
{
