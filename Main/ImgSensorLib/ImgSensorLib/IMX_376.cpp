...
int IMX_376:: ReadReg(USHORT startAddr, USHORT endAddr, USHORT *buf,USHORT page)
{
	return 1;
}
int IMX_376::WriteReg(USHORT startAddr, USHORT endAddr, USHORT *buf,USHORT page)
{
	return 1;
}
BOOL IMX_376::ReadOTPPage(int page, USHORT *ReadData)
{
	
	return 1;

}
int IMX_376::GetTemperature(USHORT &temperature)
{
	USHORT temperature_Flag=0;
	if(I2cRead(0x0138,&temperature_Flag,3) != 1) return 0;
	temperature_Flag=temperature_Flag|0x01;
	if(I2cWrite(0x0138,temperature_Flag,3) != 1) return 0;
	Sleep(100);

	if(I2cRead(0x013a,&temperature,3) != 1) return 0;

	temperature_Flag = temperature_Flag&0xfe;
	if(I2cWrite(0x0138,temperature_Flag,3) != 1) return 0;

	return 1;
}
int IMX_376::GetFuseID(CString &FuseID) 
{
	FuseID = _T("");
	USHORT temp1  = 0;
	int i=0;
...
int IMX_376::ApplyAWBGain(int rg, int bg, int Typical_rg, int Typical_bg)
{
...
int IMX_376::ApplyLsc(BYTE* raw8,int width,int height,int nLSCTarget,int ob,UCHAR* nLenCReg,int nLenCRegCount,int LSCGroup)
{
	return 1;
}

int IMX_376::spcCal(USHORT *imgBuf,short *pSPC)
{
	return 1;
}

int IMX_376::ApplySpc(short *pSPC)
{
	return 1;
}

int IMX_376::ReadExp( int &exp)
{
	USHORT ExpHigh;
	USHORT ExpLow;
...
int IMX_376::ReadGain( USHORT &gain)
{
	USHORT GainHigh;
	USHORT GainLow;
...
int IMX_376::WriteGain( USHORT gain)
{
