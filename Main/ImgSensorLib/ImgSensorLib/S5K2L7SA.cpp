...
int S5K2L7SA:: ReadReg(USHORT startAddr, USHORT endAddr, USHORT *buf,USHORT page)
{
	if(NULL == buf) return 0;
	if(I2cRead(startAddr,buf,3) != 1) return 0;
	return 1;
}
int S5K2L7SA::WriteReg(USHORT startAddr, USHORT endAddr, USHORT *buf,USHORT page)
{
	if(NULL == buf) return 0;
	if(I2cWrite(startAddr,*buf,3) != 1) return 0;
	return 1;
}
int S5K2L7SA::GetTemperature(USHORT &temperature)
{
	return 1;
}
int S5K2L7SA::GetFuseID(CString &FuseID) 
{
	FuseID = _T("");
	CString fuseid ;
	char section[512] = {0};
...
int S5K2L7SA::ApplyLsc(BYTE* raw8,int width,int height,int nLSCTarget,int ob,UCHAR* nLenCReg,int nLenCRegCount,int LSCGroup)
{
	return 1;
}

int S5K2L7SA::spcCal(USHORT *imgBuf,short *pSPC)
{
	return 1;
}
int S5K2L7SA::ApplySpc(short *pSPC)
{
	return 1;
}

int S5K2L7SA::ReadExp( int &exp)
{
	USHORT ExpHigh;
	USHORT ExpLow;
...
int S5K2L7SA::ReadGain( USHORT &gain)
{
	USHORT GainHigh;
	USHORT GainLow;
...
int S5K2L7SA::WriteGain( USHORT gain)
{
