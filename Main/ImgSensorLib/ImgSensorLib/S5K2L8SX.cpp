...
int S5K2L8SX:: ReadReg(USHORT startAddr, USHORT endAddr, USHORT *buf,USHORT page)
{
	if(NULL == buf) return 0;
	if(I2cRead(startAddr,buf,3) != 1) return 0;
	return 1;
}
int S5K2L8SX::WriteReg(USHORT startAddr, USHORT endAddr, USHORT *buf,USHORT page)
{
	if(NULL == buf) return 0;
	if(I2cWrite(startAddr,*buf,3) != 1) return 0;
	return 1;
}
int S5K2L8SX::GetTemperature(USHORT &temperature)
{
	return 1;
}
int S5K2L8SX::GetFuseID(CString &FuseID) 
{
	FuseID = _T("");
	CString fuseid ;
	char section[512] = {0};
	...
	USHORT tempVal[64];
	USHORT readData =0;
	if(I2cRead(0x0100,&readData,4) != 1) return 0;
	if (readData != 0x0100)
	{
		if(I2cWrite(0x0100, 0x0100, 4) != 1) return 0;
	}
...
int S5K2L8SX::ApplyAWBGain(int rg, int bg, int Typical_rg, int Typical_bg)
{
...
int S5K2L8SX::ApplyLsc(BYTE* raw8,int width,int height,int nLSCTarget,int ob,UCHAR* nLenCReg,int nLenCRegCount,int LSCGroup)
{
	return 1;
}

int S5K2L8SX::spcCal(USHORT *imgBuf,short *pSPC)
{
	return 1;
}
int S5K2L8SX::ApplySpc(short *pSPC)
{
	return 1;
}

int S5K2L8SX::ReadExp( int &exp)
{
	USHORT ExpHigh;
	USHORT ExpLow;
...
int S5K2L8SX::ReadGain( USHORT &gain)
{
	USHORT GainHigh;
	USHORT GainLow;
...
int S5K2L8SX::WriteGain( USHORT gain)
{
