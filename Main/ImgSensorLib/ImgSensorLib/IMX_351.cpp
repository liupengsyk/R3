...
int IMX_351:: ReadReg(USHORT startAddr, USHORT endAddr, USHORT *buf,USHORT page)
{
	return 1;
}
int IMX_351::WriteReg(USHORT startAddr, USHORT endAddr, USHORT *buf,USHORT page)
{
	return 1;
}
BOOL IMX_351::IMX351_ReadOTPPage(int page, USHORT *ReadData)
{
...
int IMX_351::GetTemperature(USHORT &temperature)
{
	return 1;
}
int IMX_351::GetFuseID(CString &FuseID) 
{
	FuseID = L"";
...
int IMX_351::ApplyAWBGain(int rg, int bg, int Typical_rg, int Typical_bg)
{
...
int IMX_351::ApplyLsc(BYTE* raw8,int width,int height,int nLSCTarget,int ob,UCHAR* nLenCReg,int nLenCRegCount,int LSCGroup)
{
	return 1;
}

int IMX_351::spcCal(USHORT *imgBuf,short *pSPC)
{
	return 1;
}

int IMX_351::ApplySpc(short *pSPC)
{	
	...
	for (int i=0; i<70; i++)//48
	{
		...
		if(I2cWrite(0x7548+i, pSPC[i+70], 3) != 1) return 0;
	}
	Sleep(1);
...
int IMX_351::ReadGain( USHORT &gain)
{
	USHORT GainHigh;
	USHORT GainLow;
...
int IMX_351::WriteGain( USHORT gain)
{
