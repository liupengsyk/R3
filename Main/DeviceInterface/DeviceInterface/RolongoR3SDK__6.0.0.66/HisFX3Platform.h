...
public:
	...
	\param usPID USB3.0设备PID， 此参数暂时保留，请固定设为0
	\param usVID USB3.0设备VID， 此参数暂时保留，请固定设为0
	\param pstrFriendlyName USB3.0设备描述名称， 此参数暂时保留，请固定设为0
	\param ucDeviceIndex USB3.0设备序号， 此参数暂时保留，请固定设为0xFF
	\param puiKey 工装唯一序列号， 当puiKey==NULL,函数打开连接到电脑的任一工装； 
	...
	*/
	int HisFX3OpenDevice(unsigned short usPID=0, unsigned short usVID=0, char* pstrFriendlyName=NULL, unsigned char ucDeviceIndex=0xFF, unsigned int* puiKey = NULL, unsigned int uiTransformDelay = 170);
	...
	*/
	...
	\return 0:成功  非0:失败的错误代码, 请参考_HisFX3Platform_ErrorCode
	\sa HisFX3OpenDevice()， HisFX3CloseDevice(), _HisFX3Platform_ErrorCode
	*/
	...
	*/
	int HisFX3PageWriteIIC(unsigned char ucSlave, unsigned int uiReg, unsigned int uiRegSize, unsigned char* pucData, unsigned int uiDataSize, unsigned short usDelay = 2);
	...
	*/
	int HisFX3PageWriteIIC_S2(unsigned char ucSlave, unsigned int uiReg, unsigned int uiRegSize, unsigned char* pucData, unsigned int uiDataSize, unsigned short usDelay = 0);
	...
	*/
	...
	//! 设置通道1电压
	/*!    
	\param[in] ucDOVDD 0.0V~3.5V范围可调, eg:2.8V, 应设为28
	\param[in] ucAVDD 0.0V~3.5V范围可调, eg:2.8V, 应设为28
	\param[in] ucDVDD 0.0V~3.5V范围可调, eg:2.8V, 应设为28
	\param[in] ucAF 0.0V~3.5V范围可调, eg:2.8V, 应设为28
	\param[in] ucVFuse 0.0V~10.0V范围可调, eg:2.8V, 应设为28
	\param[in] ucPOW 0.0V~3.5V范围可调【R5】【R3X】【R5X】, eg:2.8V, 应设为28
	\param[in] ucIODD 0.0V~3.5V范围可调【板卡内部IO电平】【R5】, eg:2.8V, 应设为28
	\param[in] ucSwitch 设置电压开关， _HisFX3_Platform_VoltFlag，
	...
	*/
	int HisFX3SetVolt(unsigned char ucDOVDD=0, unsigned char ucAVDD=0, unsigned char ucDVDD=0, unsigned char ucAF = 0, unsigned char ucVFuse=0, unsigned char ucPOW=0, unsigned char ucIODD = 0, unsigned char ucSwitch=0xFF);
	...
	\param[in] ucAF 0.0V~3.5V范围可调, eg:2.8V, 应设为28
	\param[in] ucVFuse 0.0V~10.0V范围可调, eg:2.8V, 应设为28
	\param[in] ucPOW 0.0V~3.5V范围可调【R5】, eg:2.8V, 应设为28
	\param[in] ucIODD 0.0V~3.5V范围可调【板卡内部IO电平】【R5】, eg:2.8V, 应设为28
	\param[in] ucSwitch 设置电压开关， _HisFX3_Platform_VoltFlag，
									  eg,设置AVDD和DVDD电压， _HisFX3_Platform_VlotOn_AVDD | _HisFX3_Platform_VlotOn_DVDD
	\return 0:成功  非0:失败 请参考_HisFX3Platform_ErrorCode
	...
	*/
	int HisFX3SetVolt_S2(unsigned char ucDOVDD=0, unsigned char ucAVDD=0, unsigned char ucDVDD=0, unsigned char ucAF = 0, unsigned char ucVFuse=0, unsigned char ucPOW=0, unsigned char ucIODD = 0, unsigned char ucSwitch=0xFF);

	//! 获取通道1当前设置电压
	/*!    
	\param[out] pucDOVDD 当前DOVDD设置电压， 实际电压 = *pucDOVDD /10.0
	\param[out] pucAVDD 当前AVDD设置电压， 实际电压 = *pucAVDD /10.0 
	\param[out] pucDVDD 当前DVDD设置电压， 实际电压 = *pucDVDD /10.0 
	\param[out] pucAF 当前AF设置电压， 实际电压 = *pucAF /10.0 
	\param[out] pucVFuse 当前VFuse设置电压， 实际电压 = *pucVFuse /10.0 
	\param[out] pucPOW 当前POW设置电压， 实际电压 = *pucPOW /10.0 
	\param[out] pucIODD 当前IODD设置电压， 实际电压 = *pucIODD /10.0 
	\return 0:成功  非0:失败 请参考_HisFX3Platform_ErrorCode
	\sa  HisFX3SetVolt,  _HisFX3Platform_ErrorCode
	*/
	int HisFX3GetVolt(unsigned char* pucDOVDD=NULL, unsigned char* pucAVDD=NULL, unsigned char* pucDVDD=NULL, unsigned char* pucAF=NULL, \
		unsigned char* pucVFuse = NULL, unsigned char* pucPOW = NULL, unsigned char* pucIODD = NULL);
	...
	*/
	int HisFX3GetVolt_S2(unsigned char* pucDOVDD=NULL, unsigned char* pucAVDD=NULL, unsigned char* pucDVDD=NULL, unsigned char* pucAF=NULL, \
		unsigned char* pucVFuse = NULL, unsigned char* pucPOW = NULL, unsigned char* pucIODD = NULL);
...
private:
	...
	int getFPGADSN(unsigned char* ucnumber);
