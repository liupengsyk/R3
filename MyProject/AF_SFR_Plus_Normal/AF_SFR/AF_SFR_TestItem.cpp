...
// 	CStdioFile myofset;
// 	if (myofset.Open(path,CFile::modeCreate|CFile::modeWrite|CFile::typeText))
// 	{
// 		myofset.Close();
// 	}
	SetConfigPath(path);	 //设置CONFIG路径	
   ...
   //DebugInfo("ErrorCode = %d",m_Options.ErrorCode);
	...
	memset(m_stAFData,0,sizeof(AFData)*MAXSTEPCOUNT);
	...
	if (pDlg->m_ContrastStepCount==0)
	{
		m_iStep = (m_Options.m_iEndCode - m_Options.m_iStartCode) / 6;
	} 
	else
	{
		m_iStep = (m_Options.m_iEndCode - m_Options.m_iStartCode) / pDlg->m_ContrastStepCount;
	}
	...
	m_CurrentStartCode = m_iCode;	//2017.05.11
	m_CurrentEndCode = m_iEndCode;
	m_iPeroid = 0;
	m_bAFGoing = TRUE;
	m_iLastPos = 0;
   ...
   //TODO:在此添加测试项代码
	...
	else if (m_nJumpPos == 0)     //对焦
	{
		...
		if (m_Options.m_iAFMethod == 0 || m_iPeroid == 2)
		{
			...
			memset(&m_ofDestDataOut,0,sizeof(OFAutoROIOut));
			m_pInterface->Ctrl_Addlog(CamID,_T("AF Auto ROI....."),COLOR_BLUE,200);
			...
			{
				m_iROIFailCount = 0;
				m_pInterface->Ctrl_Addlog(CamID,_T("AF SFRTest....."),COLOR_BLUE,200);
				BOOL bSFRResult = SFRTest(m_ofDestDataOut,m_stROIInfo,m_iROICount);
				m_pInterface->Ctrl_Addlog(CamID,_T("AF SFRTest....."),COLOR_BLUE,200);
				if (!bSFRResult)
				{
					CString temp;
		...
		if (bStop)
		{
			...
			m_pInterface->Ctrl_Addlog(CamID, temp, COLOR_BLUE, LOGHEIGHT);
			if ((m_iPeakDAC <=  m_Options.m_iStartCode || m_iPeakDAC >= m_Options.m_iEndCode||m_iPeakDAC <=  m_CurrentStartCode || m_iPeakDAC >= m_CurrentEndCode)&&pDlg->m_CheckPeakCode)
			{
				m_pInterface->Ctrl_Addlog(CamID,_T("Peak code out range of [Start~End] Pos..."),COLOR_RED,LOGHEIGHT);
				m_pInterface->Ctrl_SetCamErrorInfo(CamID,_T("Peak code异常..."));
				m_pInterface->Ctrl_SetCamErrorCode(CamID, m_Options.ErrorCode);
				SetResult(RESULT_FAIL);
				return 0;
			}
			m_iStep = (m_iEndCode - m_iCode) / 6;	//m_iCode为新的StartCode
			if(pDlg->m_AFTestMode == NORMAL_MODE)
			{
				if (m_iPeroid == 0)
				{
					...
					m_iLastPos = m_iPos;
					m_iPeroid = 2;
					m_Options.m_iAFMethod = 0;//转为SFR对焦
					m_CurrentStartCode = m_iCode;		//2017.05.11
					m_CurrentEndCode = m_iEndCode;
	...
	else if (m_nJumpPos == 2)	    //Peak位置SFR计算
	{
		...
		{	
			...
			{
				CStdioFile MyRawPictrue;
				CString FileName_SFRTest;
				FileName_SFRTest.Format(_T("D:\\Raw10_AutoROI_Cam%d.raw"),CamID);
				if (MyRawPictrue.Open(FileName_SFRTest,CFile::modeCreate|CFile::modeWrite|CFile::typeBinary))
				{
					MyRawPictrue.Write(img.Raw_buffer,img.imgheight*img.imgwidth*2);
					MyRawPictrue.Close();
				}
			}
			m_pInterface->Ctrl_Addlog(CamID,_T("AutoROI开始"),COLOR_BLUE,200);
			BOOL ret = AutoROI(m_ofDestDataOut);
			m_pInterface->Ctrl_Addlog(CamID,_T("AutoROI结束"),COLOR_BLUE,200);
			if (ret)
			{
				m_iBlockCount = m_ofDestDataOut.ofROInumber;
				m_bFinalCheck = TRUE;
		...
		if (pDlg->m_bSaveImage)
		{
			CStdioFile MyRawPictrue;
			CString FileName_SFRTest;
			FileName_SFRTest.Format(_T("D:\\Raw10_SFRTest_Cam%d.raw"),CamID);
			if (MyRawPictrue.Open(FileName_SFRTest,CFile::modeCreate|CFile::modeWrite|CFile::typeBinary))
			{
				MyRawPictrue.Write(img.Raw_buffer,img.imgheight*img.imgwidth*2);
				MyRawPictrue.Close();
		...
		m_pInterface->Ctrl_Addlog(CamID,_T("SFR test start.."),COLOR_BLUE,200);
		BOOL bSFRResult = SFRTest(m_ofDestDataOut,stROIInfo,m_iROICount);
		m_pInterface->Ctrl_Addlog(CamID,_T("SFR test end.."),COLOR_BLUE,200);
		if (!bSFRResult)
		{
			m_iFinalSFRTest++;
			CString temp;
		...
		m_pInterface->Ctrl_Addlog(CamID,_T("draw result start.."),COLOR_BLUE,200);
		DrawText(stROIInfo,m_iROICount,m_iBlockCount);
		m_pInterface->Ctrl_Addlog(CamID,_T("draw result end.."),COLOR_BLUE,200);
...
{
	pDlg->m_CheckPeakCode = ReadConfigInt(_T("CheckPeakCode"),1);
	pDlg->m_ContrastStepCount = ReadConfigInt(_T("ContrastStepCount"),6);
	pDlg->m_bCTCLimit = ReadConfigInt(_T("CTCLimit"),0);
	...
	pDlg->m_smallRange = ReadConfigString(_T("SFR_SMALL_RANGE"), _T("20"));
	//leger add
	...
	temp = 	ReadConfigString(_T("SFR_TEST_FREQ"),_T("0"));	
	pDlg->m_sfrFreq = (float)_tcstod(temp,0);
	...
	m_Options.m_iStep1 = ReadConfigInt(STEP1,30);
	m_Options.m_iStep2 = ReadConfigInt(STEP2,15);
...
int AF_SFR_TestItem::SaveOption()
{
	...
	//TODO: 在此添加存储代码
	WriteConfigInt(_T("ContrastStepCount"),pDlg->m_ContrastStepCount);
	WriteConfigInt(_T("CTCLimit"),pDlg->m_bCTCLimit);

	WriteConfigInt(_T("CheckPeakCode"),pDlg->m_CheckPeakCode);
	...
	temp.Format(_T("%.6f"),pDlg->m_sfrFreq);
	WriteConfigString(_T("SFR_TEST_FREQ"),temp);
...
BOOL AF_SFR_TestItem::CheckFinalSpec()
{
	...
	do 
	{
		...
		if (pDlg->m_bCTCLimit)
		{
			...
			m_fCTC8 = (m_fCTC8_H>m_fCTC8_V)?m_fCTC8_V:m_fCTC8_H;
			m_fCTC6 = (m_fCTC6_H>m_fCTC6_V)?m_fCTC6_V:m_fCTC6_H;
			m_fCTC5 = (m_fCTC5_H>m_fCTC5_V)?m_fCTC5_V:m_fCTC5_H;
			tempLog.Format(_T("CTC8=%.3f  CTC8_H=%.3f  CTC8_V=%.3f"),m_fCTC8,m_fCTC8_H,m_fCTC8_V);
			m_pInterface->Ctrl_Addlog(CamID,tempLog,COLOR_BLUE,200);
			tempLog.Format(_T("CTC6=%.3f  CTC6_H=%.3f  CTC6_V=%.3f"),m_fCTC6,m_fCTC6_H,m_fCTC6_V);
			m_pInterface->Ctrl_Addlog(CamID,tempLog,COLOR_BLUE,200);
			tempLog.Format(_T("CTC5=%.3f  CTC5_H=%.3f  CTC5_V=%.3f"),m_fCTC5,m_fCTC5_H,m_fCTC5_V);
			m_pInterface->Ctrl_Addlog(CamID,tempLog,COLOR_BLUE,200);
		}
...
float AF_SFR_TestItem::GetSFRValue(unsigned char *_InImageBuffer,ROIInfo roiinfo,int ROISize,wchar_t *stringinfo,int index,int blockIndex)
{
...
{	
	...
	char ErrorMsg[512]={0};
	BOOL bAutoROI = FALSE;
// 	CString FileName=m_pInterface->Ctrl_GetCurrentPath();
// 	FileName += _T("ImgSave\\");
// 	CreateDirectory(FileName,NULL);
// 	if (CamID==0)
// 	{
// 		FileName += _T("AutoROI_Cam0.bmp");
// 	} 
// 	else
// 	{
// 		FileName += _T("AutoROI_Cam1.bmp");
// 	}
// 	bSaveBmpFile(FileName,BMPBuffer,img.imgwidth,img.imgheight);
	if (m_bFocusing)
	{
		if (m_Options.bEngineerMode)
		{
	...
	{
		m_pInterface->Ctrl_Addlog(CamID,_T("call dll start...."),COLOR_BLUE,200);
		bAutoROI = fnAutoROISelect(&ofSrcDataIn, &ofDestDataOut,ErrorMsg);
		m_pInterface->Ctrl_Addlog(CamID,_T("call dll end...."),COLOR_BLUE,200);
	}
...
{	
	...
	//m_pInterface->Ctrl_Addlog(CamID,_T("P10Buffer OK"),COLOR_BLUE,200);
	if (m_bFinalCheck&&pDlg->m_bSaveImage)
	{  
		char filename[1024]  = {0};
		CString temppath = _T("");
	...
	for (int i = 0; i < iROINum; i++)
	{
		if ((ofDestDataOut.ofROIposition[i]._y1+ofDestDataOut.ofROIposition[i]._y2) == 0)
		{
			...
			if (_sfrC+_sfrD)
			{
				...
				sfrvalue1 = GetSFRValue(P10Buffer,ofDestDataOut.ofROIposition[i].ROIH1,ofDestDataOut.ofROISize,L"H1",nCount,i);
				sfrvalue2 = GetSFRValue(P10Buffer,ofDestDataOut.ofROIposition[i].ROIH2,ofDestDataOut.ofROISize,L"H2",nCount,i);
		...
		else 
		{
			...
			sfrvalue1 = GetSFRValue(P10Buffer,ofDestDataOut.ofROIposition[i].ROIH1,ofDestDataOut.ofROISize,L"H1",nCount,i);
			sfrvalue2 = GetSFRValue(P10Buffer,ofDestDataOut.ofROIposition[i].ROIH2,ofDestDataOut.ofROISize,L"H2",nCount,i);
			sfrvalue3 = GetSFRValue(P10Buffer,ofDestDataOut.ofROIposition[i].ROIH3,ofDestDataOut.ofROISize,L"H3",nCount,i);				
			sfrvalue4 = GetSFRValue(P10Buffer,ofDestDataOut.ofROIposition[i].ROIH4,ofDestDataOut.ofROISize,L"H4",nCount,i);
		...
		if ((ofDestDataOut.ofROIposition[i]._x1+ofDestDataOut.ofROIposition[i]._x2) == 0)
		{
			...
			if (_sfrA+_sfrB)
			{
				...
				sfrvalue1 = GetSFRValue(P10Buffer,ofDestDataOut.ofROIposition[i].ROIV1,ofDestDataOut.ofROISize,L"V1",nCount,i);
				sfrvalue2 = GetSFRValue(P10Buffer,ofDestDataOut.ofROIposition[i].ROIV2,ofDestDataOut.ofROISize,L"V2",nCount,i);
				if (m_bFinalCheck&&pDlg->m_bSaveImage)
				{
		...
		else 
		{
			...
			sfrvalue1 = GetSFRValue(P10Buffer,ofDestDataOut.ofROIposition[i].ROIV1,ofDestDataOut.ofROISize,L"V1",nCount,i);
			sfrvalue2 = GetSFRValue(P10Buffer,ofDestDataOut.ofROIposition[i].ROIV2,ofDestDataOut.ofROISize,L"V2",nCount,i);
			sfrvalue3 = GetSFRValue(P10Buffer,ofDestDataOut.ofROIposition[i].ROIV3,ofDestDataOut.ofROISize,L"V3",nCount,i);				
			sfrvalue4 = GetSFRValue(P10Buffer,ofDestDataOut.ofROIposition[i].ROIV4,ofDestDataOut.ofROISize,L"V4",nCount,i);
			if (m_bFinalCheck&&pDlg->m_bSaveImage)
			{
...
BOOL AF_SFR_TestItem::DrawText(OfMarker* pStROIInfo,int iROICount,int iBlockCount)
{
	...
	{
		int x = stROIPos[i].x - ImgWidth/16;
		int y = stROIPos[i].y - ImgHeight/12;
		int sx = ImgWidth/8;
		int sy = ImgHeight/6;
...
BOOL AF_SFR_TestItem::SaveSFR()
{
	...
	USHORT driverTemperature = 0;
	GetTemperature_Driver(&driverTemperature);
	temp.Format(_T("%d"),(int)(driverTemperature));
	WritePrivateProfileString(m_Options.m_StationName,_T("TEMPERATURE_Driver"),temp,strFileName);
	strLog.Format(_T("Temperature_Driver = %d"), driverTemperature);
	m_pInterface->Ctrl_Addlog(CamID, strLog, COLOR_BLUE, 200);
...
int AF_SFR_TestItem::GetBestCode(CString afMode, AF_STEP_DATA stepData, int stepCode, int* startCode, int* endCode, float* bestAv)
{
	int ret = 0;
	...
	else
	{
		...
		{
			m_pInterface->Ctrl_Addlog(CamID,_T("peak code at first"),CamID,200);
		...
		{
			m_pInterface->Ctrl_Addlog(CamID,_T("peak code at end"),CamID,200);
