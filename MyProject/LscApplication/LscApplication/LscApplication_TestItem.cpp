#include "StdAfx.h"
#include "LscApplication_TestItem.h"
#include "ImageProc.h"
...
extern "C" Tccm_EXP TestItemInterface * CreateTestItem(ccmBaseInterface* pInterface, LPCTSTR lpszName)
{
	...
	return new LscApplication_TestItem(pInterface, lpszName);
}
...
*******************************************************************/
LscApplication_TestItem::LscApplication_TestItem(ccmBaseInterface* pInterface, LPCTSTR lpszName)
{
...
LscApplication_TestItem::~LscApplication_TestItem(void)
{

}


int LscApplication_TestItem::InitItem()
{
	CamID = GetCamID();
	CurrentName=GetName();
...
返回值：   0 - 完成   非0 - 未完成
*******************************************************************/
int LscApplication_TestItem::Initialize()
{
...
返回值：   0 - 完成   非0 - 未完成
*******************************************************************/
int LscApplication_TestItem::Testing()
{
   //TODO:在此添加测试项代码
	if (pDlg->m_QLSC)
	{
...
返回值：   0 - 完成   非0 - 未完成
*******************************************************************/
int LscApplication_TestItem::Finalize()
{
	//TODO:在此添加回收结束代码
...
返回值：   无返回值
*******************************************************************/
void LscApplication_TestItem::MTKLSC_Paramter(CString Filepath)
{
...
返回值：   0 - 成功   非0 - 不成功
*******************************************************************/
int LscApplication_TestItem::LoadOption()
{
	//TODO: 在此添加读取代码 
...
返回值：   0 - 成功   非0 - 不成功
*******************************************************************/
int LscApplication_TestItem::SaveOption()
{
...
返回值：   字符串指针
*******************************************************************/
LPCTSTR LscApplication_TestItem::GetReportHeads()
{
...
返回值：   字符串指针
*******************************************************************/
LPCTSTR LscApplication_TestItem::GetReportLowLimit()
{
...
返回值：   字符串指针
*******************************************************************/
LPCTSTR LscApplication_TestItem::GetReportHightLimit()
{
...
返回值：   字符串指针
*******************************************************************/
LPCTSTR LscApplication_TestItem::GetReportContents()
{
...
返回值：   返回 1 成功        返回0 失败
*******************************************************************/
BOOL LscApplication_TestItem::MTK_LSCCal()
{
	USES_CONVERSION;
...
返回值：   返回 1 成功        返回0 失败
*******************************************************************/
BOOL LscApplication_TestItem::MTKMONO_LSCCal()
{
	//TODO:在此添加测试项代码
	USES_CONVERSION;
...
void LscApplication_TestItem::SaveColorShadingData(float * Color_RG , float * Color_BG , BOOL Result)
{
    USES_CONVERSION;
...
返回值：   返回 1 成功        返回0 失败
*******************************************************************/
BOOL LscApplication_TestItem::LSCDataCheck(USHORT *m_LSCData)
{
	USES_CONVERSION;
	FILE *fp = NULL;
    ...
    sfilename.Format(_T("%s\\LSC\\Qualcomm\\Standard_%d.ini"), m_pInterface->Ctrl_GetCurrentPath(), CamID);
	...
	for( int i = 0; i < 221; i++ )
	{
		...
		tempGrGbDiff = abs(tempBuf[i][1]-tempBuf[i][2]);
	...
	{
		TempMsg.Format(_T("GrGb Diff = %d > %d !"),GrGbDiff,pDlg->m_GrGbDiff);
		m_pInterface->Ctrl_Addlog(CamID,TempMsg,COLOR_RED,200);
		return 0;
	}

	SaveQualLSCData(tempBuf);
	...
	m_pInterface->Ctrl_Addlog(CamID,TempMsg,COLOR_BLUE,200);
	   ...
	   FILE  *fpn  = NULL;
	   CString FileName = _T("");
       FileName.Format(_T("%s\\LSC\\Qualcomm\\%d\\LSCData.txt"), m_pInterface->Ctrl_GetCurrentPath(), CamID);
...
返回值：   返回 1 成功        返回0 失败
*******************************************************************/
BOOL LscApplication_TestItem::Qualcomm_LSCCal()
{
	...
	m_szDirectory=m_pInterface->Ctrl_GetCurrentPath()+_T("LSC\\Qualcomm\\");
    ...
    m_szDirectory.Format(_T("%s%d\\"), m_szDirectory.GetBuffer(), CamID);
...
BOOL LscApplication_TestItem::Sensor_LSCCal()
{
	ImageInfo *MyImageInfo;
	MyImageInfo=m_pInterface->Ctrl_GetImgInfo(CamID);
	if (NULL == MyImageInfo->Raw_buffer)
...
返回值：   返回 1 成功        返回0 失败
*******************************************************************/
BOOL LscApplication_TestItem::ReadDataFromTXT(float *R_Channel, float *Gr_Channel, float *Gb_channel, float *B_Channel)
{
	USES_CONVERSION;
	int mycharlength = 0;
	int strlength = 0;
	...
	CStdioFile file;
	CString Path;
    Path.Format(_T("%s\\LSC\\Qualcomm\\%d\\AWB_LSC_CALIBRATION_DATA.txt"), m_pInterface->Ctrl_GetCurrentPath(), CamID);
...
BOOL LscApplication_TestItem::ReadBinData(USHORT *OTPData)
{
	HANDLE hFile;
	int Timeout=100;
	CString eeprom_slim;
	eeprom_slim.Format(_T("%s\\LSC\\Qualcomm\\%d\\ONLY_LSC_CALIBRATION_DATA_BYTE_FORMAT.txt"), m_pInterface->Ctrl_GetCurrentPath(), CamID);
...
void LscApplication_TestItem::SaveQualLSCData(int  LSCData[][4])
{
	USES_CONVERSION;
	...
	TCHAR szReportPath[MAX_PATH] = {0};
	CString strReportPath = ReadConfigString(_T("sReportPath"), szReportPath, MAX_PATH);
	FileName.Format(_T("%s\\QualLSC_Test_Site_A%04d%02d%02d.xls"), strReportPath.GetBuffer(), time.GetYear(), time.GetMonth(), time.GetDay()); ;
	...
	if(fileSizeTemp==0)
	{
		printf("FUSE ID\tTest Time\tChannel\t");
		for (i = 0; i < 221; i++)
		{
			printf( "%s%d\t", "Block_", i+1);
		}	
	}
